...
function BillboxList() {
  ...
  const handleFindALL = async () => {
    ...
    const res = privateAxios
      .get(
        `/charge/getBillBoxDetail?pageNumber=${
          pageNumber - 1
        }&pageSize=${pageSize}`
      )
      .then((res) => {
        ...
        if (pageNumber !== "") {
          ...
          if (pageSize > res.data.pageList.content?.length) {
            setLastIndex(
              Math.max(
                (pageNumber + 1) * res.data.pageList.content?.length,
                res.data.pageList.totalElements
              )
            );
          } else {
          ...
          <table className="table userTable">
            ...
            <tbody>
              <FindBillBoxData
                ...
                select3Data={select3Data}
                allData={allData}
                setAllData={setAllData}
                handleFindALL={handleFindALL}
              />
            </tbody>
          </table>
        </div>
