...
//   },[uid])
  ...
  const handleDeleteData=async(id)=>{
    ...
    if(conf){
        ...
        try{
            const res = await axios.get(`finYear/delete/${id}`)
  ...
  const handleSubmit=async(event)=>{
        ...
        try{
          const res = await axios.put(`finYear/update/${uid}`, dataForUpdate );
  ...
  const handleInputChange = (event) => {
    ...
    setAllData((prevFinYear) => ({ ...prevFinYear, [name]: value }));
};
            ...
            return (
                <tr >
                    ...
                    <td><button type="button" class="btn btn-danger" onClick={()=>handleDeleteData(finYearId)}>Delete</button>
                        <button type="button" class="btn m-1 btn-light" onClick={()=>handleUpdateData(finYearId)}>Update</button>
                    </td>
                </tr>
            )
        })
